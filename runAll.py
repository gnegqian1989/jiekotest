...
def run(self):
    try:
        suit = self.set_cause_suite()
        if suit is notNone:
            logger.info("******TEST START*******")
            fp = open(resultPath,'wb')
            runner = HTMLTestRunner.HTMLTestRunner(stream = fp, title = 'Test Report',decription = 'Test Description')
            runner.run(suit)
        else:
        ...
        if init(on_off) == 0:
            ...
            logger.info("Doesn'tsend report email to develpoer")
        else:
